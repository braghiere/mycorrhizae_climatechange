...
#Sulman et al. 2019
surf_ori = Dataset('/home/renato/datasets/sulman_2019/Global-data/perecm_obs_Sulman.nc','r')
...
#surf_ori = Dataset('/home/renato/datasets/soudzilovskaia_2019/MycDistrEM_current_all.nc','r')
...
#Steidinger
perecm_ori = surf_ori.variables['perecm'][:]
#perecm_ori = surf_ori.variables['Band1'][:]
...
lat = np.arange(-89.9,90.0,0.16666666666666666667)
...
perecm_ori_plus = np.empty((1080,2161))

for i in xrange(1080):
   if i < 37:
        perecm_ori_plus[i,:] = perecm_ori[839,:]
   if 37 < i < 836:
        perecm_ori_plus[i,:] = perecm_ori[i-37,:]  
   else:
        perecm_ori_plus[i,:] = perecm_ori[839,:]
...
x2 = np.linspace(x[0][0],x[0][-1],x.shape[1]*1)
y2 = np.linspace(y[0][0],y[-1][0],y.shape[0]*1)
...
for i in xrange(1):
   ...
   plt.savefig('ecm_orig_sulman_0p16x0p16_larger_v2.1_v2.png',bbox_inches="tight")
   plt.show()
